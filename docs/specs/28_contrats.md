...
- Base lifecycle: draft -> under_review -> approved -> sent -> signed -> active -> completed -> archived. Termination path: active -> terminated -> archived; rejection path: under_review|sent -> rejected -> draft for redraft.
...
- Templates are referenced by id and version; generation produces a draft contract with pre-filled fields tied to project/mission identifiers.
...
- Assignments reference the active contract version for the mission; no assignment may reference multiple contracts.
